...
export default function Home() {
  ...
  const totalBudget = (monthlyBudget || 0) + (additionalIncomes || []).reduce((sum: any, inc: any) => sum + (inc?.amount || 0), 0);
  const spentAmount = (expenses || []).reduce((sum: any, exp: any) => sum + (exp?.amount || 0), 0);
  ...
  const chartData = (categories || [])
    ...
    .map((cat: any) => ({
      category: cat.name,
      amount: (expenses || [])
        .filter((exp: any) => exp && exp.category === cat.name)
        .reduce((sum: any, exp: any) => sum + (exp?.amount || 0), 0),
        ...
        <Card className="bg-white border shadow-sm">
          <div className="p-6">
            ...
            <div className="space-y-4">
              <ErrorBoundary>
                <ExpenseList 
                  expenses={expenses?.filter(exp => exp && exp.id).map(exp => ({ 
                    ...exp, 
